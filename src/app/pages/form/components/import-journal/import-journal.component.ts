...
export class ImportJournalComponent {
  ...
  uploadData() {
    ...
    reader.onload = (e) => {
      ...
      this.http
        .post(`${environment.url}transaction/import-journal`, jsonData)
        .subscribe(
          (response) => {
  ...
  upload() {
    ...
    reader.onload = (e) => {
      ...
      this.Journals = this.parseTransactions(jsonData);
      this.http
        .post(`${environment.url}transaction/import-journal`, this.Journals)
        .subscribe(
          (response) => {
            console.log("Data uploaded successfully", response);
          },
  ...
  parseTransactions(jsonData: any[]): Journal[] {
    ...
    for (let i = 1; i < jsonData.length; i++) {
      ...
      const hasNonNullValues = transactionData.some(
  ...
  parse(jsonData: any[]): Journal[] {
    ...
    for (let i = 1; i < jsonData.length; i++) {
      const transactionData = jsonData[i];
      const hasNonNullValues = transactionData.some(
        (value: string | null) => value !== null && value !== ""
      );
      if (hasNonNullValues) {
        let formattedDate = transactionData[0];
        if (transactionData[0]) {
