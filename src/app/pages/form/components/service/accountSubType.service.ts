  ...
  loading=false
    ...
    //   }
      put(data: any, id: number) {
        return this.http.put<any>(`${environment.url}${HttpApi.subType}` + id, data);
      }
      delete(id :number) {
        return this.http.delete<any>(`${environment.url}${HttpApi.PartyDelete}` + id );
      }
      deleteJournal(data: any) {
        return this.http.post<any>(`${environment.url}${HttpApi.JournalDelete}`, data );
      }
      deleteT(journal_id: any): Observable<any> {
       
        return this.http.post(`${environment.url}${HttpApi.journalDelete}` , { body: { journal_id } });
      }
      deleteParty(partyId: any): Observable<any> {
       
        return this.http.delete(`${environment.url}${HttpApi.PartyDelete}` , { body: { partyId } });
      }
      getById(id: number) {
